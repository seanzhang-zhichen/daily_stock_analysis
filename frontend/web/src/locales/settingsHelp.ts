...
const settingsHelpZhCN: SettingsHelpMap = {
  ...
  'settings.data_source.DAILY_QUOTE_SYNC_ENABLED': {
    title: '每日全量日线行情同步',
    summary: '控制定时任务触发时是否先同步全量 A 股日线行情。',
    usage: '设为 true 后，需要以 schedule 模式运行；当前版本支持 DAILY_QUOTE_SYNC_MARKETS=cn。',
    valueNotes: ['保存配置不会启动或重建当前 scheduler，需要以 schedule 模式重启后生效。'],
    impact: ['会增加收盘后的行情数据请求量，并把结果写入 stock_daily。'],
    notes: ['已存在目标交易日数据的股票会跳过。'],
  },
  'settings.data_source.DAILY_QUOTE_SYNC_MARKETS': {
    title: '每日行情同步市场',
    summary: '全量日线行情同步的市场范围。',
    usage: '当前仅支持 cn，后续扩展港股/美股前需要先定义全量范围。',
    valueNotes: ['多个市场使用英文逗号分隔。'],
    impact: ['决定同步任务枚举哪个市场的股票池。'],
    notes: ['不支持的市场会被跳过并记录 warning。'],
  },
  'settings.data_source.DAILY_QUOTE_SYNC_MAX_WORKERS': {
    title: '每日行情同步并发数',
    summary: '控制全量同步时同时拉取的股票数量。',
    usage: '建议从较低并发开始，避免触发数据源限流。',
    valueNotes: ['最小值为 1。'],
    impact: ['影响同步速度、数据源压力和失败率。'],
    notes: ['数据源不稳定时可降低该值。'],
  },
  'settings.data_source.DAILY_QUOTE_SYNC_LOOKBACK_DAYS': {
    title: '每日行情同步回看天数',
    summary: '每只股票同步时拉取的日线窗口。',
    usage: '用于补足停机期间缺口，并保留计算均线所需的历史窗口。',
    valueNotes: ['默认 30 天。'],
    impact: ['窗口越大，请求和写入量越高。'],
    notes: ['已有记录会通过 (code, date) upsert 更新。'],
  },
  'settings.data_source.DAILY_QUOTE_SYNC_LIMIT': {
    title: '每日行情同步限量',
    summary: '限制单次全量同步处理的股票数量。',
    usage: '0 表示不限制；正整数主要用于调试或小批量验证。',
    valueNotes: ['生产全量同步通常保持 0。'],
    impact: ['设为正整数会让同步只处理股票池前 N 只股票。'],
    notes: ['用于排查数据源问题时很方便。'],
  },
...
const settingsHelpEnUS: SettingsHelpMap = {
  ...
  'settings.data_source.DAILY_QUOTE_SYNC_ENABLED': {
    title: 'Full Daily Quote Sync',
    summary: 'Controls whether scheduled runs sync full A-share daily OHLCV first.',
    usage: 'Set true and run the app in schedule mode. The first version supports DAILY_QUOTE_SYNC_MARKETS=cn.',
    valueNotes: ['Saving this setting does not start or rebuild the current scheduler; restart in schedule mode.'],
    impact: ['Adds post-close market-data requests and writes results into stock_daily.'],
    notes: ['Stocks that already have the target trading date are skipped.'],
  },
  'settings.data_source.DAILY_QUOTE_SYNC_MARKETS': {
    title: 'Daily Quote Sync Markets',
    summary: 'Market scope for full daily quote sync.',
    usage: 'Currently only cn is supported. HK/US ranges should be defined before expansion.',
    valueNotes: ['Separate multiple markets with English commas.'],
    impact: ['Determines which stock pool the sync enumerates.'],
    notes: ['Unsupported markets are skipped with a warning.'],
  },
  'settings.data_source.DAILY_QUOTE_SYNC_MAX_WORKERS': {
    title: 'Daily Quote Sync Workers',
    summary: 'Controls how many stocks are fetched concurrently.',
    usage: 'Start low to avoid data-provider rate limits. Minimum is 1.',
    valueNotes: ['Higher values are faster but noisier under rate limits.'],
    impact: ['Affects sync speed, provider pressure, and failure rate.'],
    notes: ['Lower this value when providers are unstable.'],
  },
  'settings.data_source.DAILY_QUOTE_SYNC_LOOKBACK_DAYS': {
    title: 'Daily Quote Sync Lookback',
    summary: 'Daily-bar window fetched for each stock.',
    usage: 'Used to backfill downtime gaps and keep enough history for moving averages.',
    valueNotes: ['Default is 30 days.'],
    impact: ['Larger windows increase requests and writes.'],
    notes: ['Existing rows are updated through (code, date) upsert.'],
  },
  'settings.data_source.DAILY_QUOTE_SYNC_LIMIT': {
    title: 'Daily Quote Sync Limit',
    summary: 'Limits how many stocks one sync run processes.',
    usage: '0 means no limit. Positive values are mainly for debugging or small-batch validation.',
    valueNotes: ['Production full sync usually keeps this at 0.'],
    impact: ['A positive value syncs only the first N stocks in the pool.'],
    notes: ['Useful when investigating provider issues.'],
  },
  'settings.data_source.news_window': {
    title: 'News Window',
