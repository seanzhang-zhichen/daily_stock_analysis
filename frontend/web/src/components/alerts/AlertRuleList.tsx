              ...
              {rules.map((rule) => (
                <tr key={rule.id} className="align-top">
                  ...
                  <td className="px-3 py-3 text-xs text-secondary-text">
                    ...
                    <div className="mt-1">{formatDateTime(rule.cooldownUntil ?? undefined)}</div>
                    {hasChildTargetCooldown(rule) ? (
                      <div className="mt-1 text-muted-text">子目标见触发历史</div>
                    ) : null}
                  </td>
                  <td className="px-3 py-3 text-xs text-secondary-text">{formatDateTime(rule.updatedAt ?? rule.createdAt ?? undefined)}</td>
                  <td className="px-3 py-3">
