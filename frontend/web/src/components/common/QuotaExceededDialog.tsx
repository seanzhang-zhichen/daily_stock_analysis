...
export const QuotaExceededDialog: React.FC = () => {
  ...
  const isCreditExceeded = detail.error === 'credit_exceeded';
  const primaryAction = {
    label: isCreditExceeded ? '充值或订阅获取积分' : '升级或续费套餐',
    target: `/billing?from=${isCreditExceeded ? 'credit' : 'quota'}&kind=${encodeURIComponent(detail.kind)}`,
  };
        ...
        <div className="flex items-start justify-between gap-3">
          <div className="flex items-start gap-3">
            ...
            <div className="min-w-0">
              ...
              >
                {isCreditExceeded ? '积分不足' : `今日 ${kindLabel} 额度已用完`}
              </h3>
          ...
          {isCreditExceeded ? (
            <>
              <div className="flex items-center justify-between">
                <span className="text-secondary-text">当前积分</span>
                <span className="font-medium tabular-nums text-foreground">{detail.balance ?? detail.remaining}</span>
              </div>
              <div className="flex items-center justify-between">
                <span className="text-secondary-text">本次 {kindLabel} 需要</span>
                <span className="font-medium tabular-nums text-foreground">{detail.cost ?? 0}</span>
              </div>
              <p className="text-xs text-secondary-text">
                可通过订阅套餐、邀请新用户或被邀请用户付费获得积分。
              </p>
            </>
          ) : (
            <>
