...
describe('BacktestPage', () => {
  it('renders shared ui inputs and prediction tracking outputs', async () => {
    ...
    expect(screen.getByText('周期涨跌幅')).toBeInTheDocument();
  ...
  it('switches to next-day validation with the 1D shortcut', async () => {
    ...
    expect(screen.getByText('是否命中')).toBeInTheDocument();
    expect(screen.getByText('次日验证会将 AI 分析结论与下一个交易日收盘表现进行对比。')).toBeInTheDocument();
  });
});
