...
DSA 将选股能力作为主项目的一部分维护。实现参考 [AlphaSift](https://github.com/ZhuLinsen/alphasift) 提交 [`9f522747caafd3c0b1ddb7e14d5cf44c8580b6cf`](https://github.com/ZhuLinsen/alphasift/commit/9f522747caafd3c0b1ddb7e14d5cf44c8580b6cf)，并按 Apache License 2.0 修改和分发。衍生文件保留来源头，许可证位于 `backend/src/services/screening/LICENSE`，第三方声明见根目录 `THIRD_PARTY_NOTICES.md`。

## 代码边界

- `backend/src/services/screening/`：快照、日 K、策略加载、过滤、评分、风险、LLM 重排与热点实现。
- `backend/src/services/screening/strategies/`：随 DSA 版本发布的策略 YAML。
- `backend/src/services/screening/pipeline.py`：筛选流程的直接入口。
- `backend/src/services/screening_service.py`：DSA 业务编排，直接调用 pipeline，负责配置、数据源上下文、响应归一化、缓存与错误映射。
- `backend/src/storage/`：使用 DSA 现有 SQLAlchemy 基础设施持久化已完成的选股运行，不另建文件数据库。
- `backend/api/v1/endpoints/screening.py`：`/api/v1/screening` API。
- `frontend/web/src/api/screening.ts` 与 `StockScreeningPage.tsx`：Web 调用与展示。

服务层静态调用 `screening.pipeline`、`screening.strategy` 和 `screening.hotspot`。核心逻辑不通过模块名探测、动态适配器或多套路由分发，因此代码结构、错误边界和打包收集目标均由主项目直接定义。

## 配置

选股是平台默认能力，不需要配置开关。服务启动后即可使用策略、热点、筛选和历史接口；如果策略文件、数据依赖或运行环境异常，选股页面会显示具体的可用性诊断。
...
以上 API 均使用 DSA 现有登录认证。已完成的选股运行与历史查询按当前 `user_id` 隔离，用户不能读取其他用户的运行记录。
...
| 位置 | 解决的问题 | 加载方 | 执行阶段 |
| --- | --- | --- | --- |
| `backend/src/services/screening/strategies/*.yaml` | 从全市场筛出哪些候选 | `backend/src/services/screening/strategy.py` | 快照过滤、因子评分、风险和排序 |
| `backend/strategies/*.yaml` | 对单只股票如何分析和形成结论 | `backend/src/agent/skills/base.py` | DSA Agent/报告分析 |
...
| 配置与 API 更名 | 旧自动化需同步调整 | 在发布说明明确 `/api/v1/screening` |
...
2. 比较 `backend/src/services/screening/` 的 DSA 特有修改，按模块选择性移植；
...
- 业务回滚：修复或回滚选股引擎代码并重启；普通个股分析、报告、通知和问股不受影响。
