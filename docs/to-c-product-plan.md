...
> 套餐权益以 `app_plans` 为准，免费档同样通过运营后台「套餐与用量」或数据库中的 `code=free` 行维护。
> 积分包使用独立的 `app_credit_packages` / `app_credit_orders` 表和 `/api/v1/credits/*` 接口；支付通道可复用微信 / 支付宝 gateway，但支付成功后只增加用户积分，不改变订阅档位。默认内置 19.9 元 / 200 积分、99.9 元 / 1200 积分、299 元 / 4000 积分三档，后续可由运营在数据库中调整。
...
- **平台 Key 成本超预算** → Plan B：在运营后台临时下调免费档配额，或直接关闭免费档注册（仅邀请制）。
