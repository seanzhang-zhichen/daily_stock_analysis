...
当前运行时告警由 `backend/src/services/alert_worker.py` 中的后台 worker 统一调度，基础行情规则评估复用 `backend/src/services/alert_service.py` 与 `backend/src/agent/events.py` 中的 EventMonitor 规则模型，技术指标、持仓风险和 Market Light 规则由对应 service 扩展。

- 配置入口：`AGENT_EVENT_MONITOR_ENABLED`、`AGENT_EVENT_MONITOR_INTERVAL_MINUTES`、`AGENT_EVENT_ALERT_RULES_JSON`。
- 运行入口：`backend/main.py` 在 schedule 模式中注册 `agent_event_monitor` 后台任务；后台 worker 每轮读取持久化 active rules，并继续兼容 legacy `AGENT_EVENT_ALERT_RULES_JSON`。
- 通知投递：触发后复用 `NotificationService.send(..., route_type="alert")`，继续遵守通知网关的 alert 路由配置。
- Web/System 配置校验：`backend/src/services/system_config_service.py` 会对 `AGENT_EVENT_ALERT_RULES_JSON` 做 JSON 与规则语义校验。

legacy JSON runtime 支持三类规则；Alert API / Web 持久化规则还支持 P5-P7 说明的技术指标、持仓风险和 Market Light 类型：
...
- legacy JSON 规则使用进程内 fingerprint 避免持续触发条件重复推送；持久化规则使用 `alert_cooldowns`，其状态可跨进程和重启保留。
...
当前仓库已有 SQLAlchemy 存储层和 repository/service 分层：

- `backend/src/storage/` 管理 SQLAlchemy ORM 模型、`DatabaseManager` 和数据库会话。
- `backend/src/storage/models/alert.py` 定义 `AlertRuleRecord`、`AlertTriggerRecord`、`AlertNotificationRecord` 与 `AlertCooldownRecord`。
- `backend/src/repositories/` 放置数据访问层，例如 `AlertRepository`、`PortfolioRepository`。
- `backend/src/services/` 放置业务服务层，例如 `AlertService`、`PortfolioService`、`PortfolioRiskService`。
- 默认数据库路径跟随现有配置，通常落在 `data/stock_analysis.db`。

告警持久化复用以上模式：storage 定义 ORM 模型，repository 封装按用户隔离的 CRUD 和查询，service 处理规则校验、评估状态、通知结果和冷却语义。`alert_cooldowns` 由 `backend/alembic/versions/20260814_add_alert_cooldowns.py` 创建；所有后续 schema 变化也必须通过 Alembic migration 完成。

如果后续 PR 需要 schema 变更，必须同时给出：

- 幂等迁移：重复启动或执行 `alembic upgrade head` 时不得破坏已有数据。
...
- 新增 API 文件：`backend/api/v1/endpoints/alerts.py`。
- 新增 schema 文件：`backend/api/v1/schemas/alerts.py`。
- API 范围：
...
P5 在现有 Alert API、Web 告警中心和 `backend/src/services/alert_worker.py` 评估链路中新增日线技术指标规则。规则仍写入 `alert_rules`，触发、降级、失败、通知结果和持久化冷却继续复用 P2-P4 的 `alert_triggers`、`alert_notifications` 与 `alert_cooldowns` 语义。
...
- `backend/src/services/alert_indicators.py` 自行归一化 OHLCV 并计算 MA、RSI、MACD、KDJ、CCI，不依赖 fetcher 预计算的 MA5/MA10/MA20。
...
- 不扩展 `backend/src/agent/events.py` 的 legacy `AlertType` 或 `_RUNTIME_SUPPORTED_ALERT_TYPES`。
...
P6 在现有 Alert API、Web 告警中心和 `backend/src/services/alert_worker.py` 评估链路中新增 `watchlist`、`portfolio_holdings`、`portfolio_account` 三类目标范围。规则仍写入 `alert_rules`，触发、降级、失败、通知结果和持久化冷却继续复用 P2-P4 的 `alert_triggers`、`alert_notifications` 与 `alert_cooldowns` 语义，不新增表或迁移。
...
- symbol 目标使用 `get_market_for_stock(normalize_stock_code(effective_target))` 构造触发时 phase；`target_scope=market` 直接用 `normalize_market_alert_region(target)`，不会把 `cn|hk|us` 当作股票代码推断；账户级无法唯一定位市场时允许 summary 落为 `unknown`。
...
P7 在现有 Alert API、Web 告警中心和 `backend/src/services/alert_worker.py` 中新增 `target_scope=market`，消费结构化 `MarketLightSnapshot`，不解析 Markdown，不扩展 legacy `AGENT_EVENT_ALERT_RULES_JSON`，不新增表。大盘复盘历史仍写一条 `analysis_history(code=MARKET, report_type=market_review)`；多市场复盘通过 `context_snapshot.market_light_snapshots` 按 region 保存本次实际复盘的快照 map。
...
本地运行 `uv run --locked python backend/main.py --schedule`、`uv run --locked python backend/main.py --serve --schedule` 或等价内置调度模式时，设置 `AGENT_EVENT_MONITOR_ENABLED=true` 后会启动后台告警 worker；`AGENT_EVENT_MONITOR_INTERVAL_MINUTES` 控制轮询间隔。
...
仓库 `docker/Dockerfile` 默认命令是 `python backend/main.py --schedule`，因此容器内只要配置 `AGENT_EVENT_MONITOR_ENABLED=true` 就会在 schedule 模式中启用告警 worker。Web/API 持久化规则依赖应用数据库；Docker 部署时需要保留 `data/` 数据库卷，避免容器重建后丢失规则、触发历史、通知尝试和冷却状态。legacy JSON 仍通过环境变量注入，不是 Docker 专用配置体系。

### GitHub Actions

当前检出版本不包含 `.github/workflows/`，因此没有 GitHub Actions 持续运行告警 worker。若后续恢复 Actions，需要单独明确 schedule 启动方式、env 映射、规则来源和持久化数据库策略；仅配置 repository Secrets / Variables 不会启动告警轮询。
...
- P0 阶段不新增 `backend/api/v1/schemas/alerts.py` 或 Alert API。
...
- P1 新增 Alert API 代码和 `alert_rules` / `alert_triggers` / `alert_notifications` 表。最小回滚方式是 revert P1 代码，并执行对应 Alembic downgrade；是否保留历史数据由维护者在回滚前确认，禁止在业务代码中手写 DDL 清理。
- P3 是 Web 和文档改动。最小回滚方式是 revert P3 PR；不会删除已有规则、触发历史或 legacy JSON 配置。
- P4 新增 `alert_cooldowns` 表并开始写入 `alert_notifications`。回滚时先停用告警 worker，再执行 `uv run --locked alembic downgrade 20260814_decision_signals` 回退 `20260814_add_alert_cooldowns.py`，最后 revert P4 代码；downgrade 会删除冷却表及其中数据，触发和通知历史仍保留。
