...
1. **daily_stock_analysis 必须已运行**：执行 `uv run --locked python backend/main.py --serve-only` 或通过 Docker 部署，使 API 长期可用
2. **openclaw 需具备 HTTP 调用能力**：如 `system.run` 执行 curl，或内置 HTTP 工具（如 api-tester 等）
3. **说明**：OpenClaw 需要访问长期运行的 API，请在服务器本机或 Docker 中运行 DSA
...
| 现象 | 可能原因 | 处理建议 |
|------|----------|----------|
| 连接失败 | DSA 未运行、端口错误、防火墙 | 确认 `uv run --locked python backend/main.py --serve-only` 已启动，检查 `DSA_BASE_URL` |
