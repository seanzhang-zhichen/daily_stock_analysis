...
| 自选股管理 | 选择关注股票，形成个人工作区。 | 自选股 CRUD、套餐上限校验、每日推送关联。 | ✅ 已落地 |
...
- 若未要求邮箱验证，注册成功后仍不会自动创建登录 session，用户需返回登录后默认进入首页。
...
**作为** 登录会员，**我希望** 随时添加、删除和查看关注股票，**以便** 后续手动分析和每日推送围绕我的关注列表工作。

**验收标准**：

- 注册页成功提示不会自动登录；登录成功后按 `redirect` 返回原目标页，无显式 `redirect` 时默认进入首页。
- 邮箱验证成功页的登录入口进入 `/login`，不再携带首次引导跳转。
- `/watchlist` 使用股票搜索添加股票，并支持删除和刷新列表；账户页提供自选股管理入口。
- 免费用户和 Pro 用户的自选股上限均由当前 `plan.maxStocks` 决定。
- 旧 `/onboarding` 链接仅作为兼容路由保留，登录后重定向到首页。

**当前实现依据**：

- 前端：`WatchlistPage`、`AccountPage`。
