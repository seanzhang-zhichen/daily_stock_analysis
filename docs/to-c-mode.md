...
- `/account`：账户信息 + 修改密码 + 通知偏好 + 退出登录，入口跳转 `/billing`、`/watchlist`、`/account/api-keys`、`/account/orders` 与 `/account/invoices`。
...
- **套餐与配额**：`quota_guard` 已接入分析与 Agent；超额返回 402 `quota_exceeded`，前端全局弹出升级 / BYOK 引导。
...
- **模型路由**：`src/users/model_router.py` 已接入 `GeminiAnalyzer` 与 `LLMToolAdapter`；Pro 用户配置有效 BYOK 凭证时优先走用户 Key，否则按 `plan.allowed_models` 过滤平台模型。
...
- `allowed_models` 具体套餐分档配置入库与平台 Key 池用量看板。
