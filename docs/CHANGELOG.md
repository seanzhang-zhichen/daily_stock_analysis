...
- [改进] Web 中英文界面切换扩展至任务中心和 A 股大盘复盘的结构化展示，历史报告正文保持原始语言。
- [改进] Web A 股情报池的来源管理、抓取和检索文案现已支持中英文界面切换。
- [改进] Agent 聊天上下文压缩增加 cost、balanced 和 long_context_raw_first 预设，未显式设置阈值时自动采用对应策略。
- [新功能] Web 新增 A 股本地情报池管理页，可初始化来源、按源或批量刷新并检索最近入库资讯。
- [改进] A 股情报池来源管理补齐管理员权限、来源启停与删除，并保留已入库的历史情报证据。
- [修复] 本地 CLI 生成后端对未声明可选配置字段的旧调用方保持 LiteLLM 默认兼容。
- [新功能] 为 DecisionSignal 增加用户反馈持久化 API，并在分析历史保存后自动回填决策信号。
- [改进] 为 DecisionSignal 增加基于历史报告快照的重评估和可审计风险 guardrail。
- [新功能] Multi-Agent specialist 模式支持隔离并发的策略执行、冲突审议/保守自审，以及可审计的统一 final action。
- [新功能] specialist 策略意见会持久化为可归因样本，并按本地日线评估结果使用样本门槛和 Beta 先验收缩生成有界动态权重。
- [改进] 分析结果在保存前统一按 80/60/40/20 评分带生成 action，并记录评分对齐与市场风险 guardrail 审计信息。
- [改进] 历史报告运行流升级为可交互 lanes、节点详情和事件筛选回放，并补齐结构性决策校准的 action 审计记录。
- [改进] API/Web 常驻调度器新增独立 watchdog 运行状态与手动触发 API，并拒绝并发启动。

- [改进] A 股分析会按盘前、盘中、午间休市、收盘竞价、盘后和非交易日生成市场阶段摘要，写入上下文快照并在通知及报告中展示中文市场状态；阶段上下文缺失时静默降级。
- [改进] A 股分析会将已获取的基本面聚合结果保留在运行期结果对象中，通知报告和 Markdown/企业微信模板可展示真实财务摘要；该上下文不写入原始分析结果载荷。
- [改进] A 股通知报告及可选 Markdown/企业微信/简报模板会展示已有的关联板块、阶段决策护栏与分析数据来源，结构化字段缺失时保持静默降级。
- [改进] A 股本地 RSS/Atom/NewsNow 情报自动刷新增加单进程互斥和一小时冷却复用；RSS/Atom 抓取支持逐跳校验的安全重定向、DNS 重绑定防护与流式响应体大小限制。
- [改进] A 股手动大盘复盘不再因周末、节假日或交易日历结果被跳过；自动调度仍遵循交易日过滤。
- [改进] A 股分析和 Agent 问股会优先复用同股代码别名的本地 RSS/Atom/NewsNow 资讯，再回退市场级资讯，并对重复来源去重。
- [新功能] Electron 桌面端右上角新增更新状态入口，仅在安全的桌面 bridge 可用时显示，可查看状态、手动检查、下载或安装更新。
- [改进] Web 中英文界面切换扩展至 A 股首页工作台、历史列表和同股趋势抽屉，报告正文继续遵循报告自身语言设置。
- [改进] A 股通知渠道统一使用 Markdown 安全分片器，按 UTF-8 字节预算分割并保护链接、行内代码和代码块，钉钉/飞书/企业微信长报告会自动分页且不外发隐藏诊断元数据。
- [新功能] A 股首页左侧工作台新增历史、自选、今日标签页，展示当日覆盖与待分析数量，并支持首页内批量分析全部或仅分析未覆盖自选股。
- [新功能] A 股历史报告新增同股趋势抽屉，可按时间比较情绪分、趋势判断和操作建议，并支持按代码清空当前用户的全部历史记录。
- [新功能] 新增 A 股本地资讯池基础能力，支持 RSS/Atom/NewsNow 来源管理、抓取、去重持久化及分析/大盘复盘 fail-open 复用。
- [改进] A 股大盘复盘现保存结构化市场数据（指数、涨跌家数、涨跌停、行业与概念榜单、资讯和正文分段），Web 历史报告提供专用复盘视图并兼容旧 Markdown 记录。
- [改进] Agent 聊天按会话持久化已验证的技能选择，后续消息可复用或显式清空该选择。
- [新功能] A 股报告生成新增受限的 Codex CLI/OpenCode CLI 后端及 LiteLLM 回退配置。
- [新功能] Web 增加可持久化的中英文界面语言切换，并接入导航与桌面/移动端工具栏。
- [改进] 为 Agent 聊天增加可选的滚动上下文压缩，保留最近轮次并通过 Alembic 持久化摘要。

- [新功能] A 股分析历史新增运行诊断摘要接口 `GET /api/v1/history/{record_id}/diagnostics`，返回脱敏 trace_id、链路状态和可复制排障文本。
- [新功能] A 股分析历史新增 `GET /api/v1/history/{record_id}/flow` 运行流快照，提供入口、数据来源、分析与报告产物泳道。
- [改进] API/Web 常驻进程的 A 股用户日程改为独立子进程执行，加入默认 45 分钟 watchdog，超时后清理工作进程而不阻断服务。
- [新功能] 新增钉钉群机器人一等通知渠道，支持可选签名和 Markdown 长文分片发送。
- [新功能] 飞书 App Bot 支持将 A 股报告作为 Markdown 文件发送，并支持 `FEISHU_DOMAIN=lark` 国际域名。
- [改进] A 股 Agent 工具调用新增显式、单工具与类别默认超时的 first-wins 策略；超时结果不可重试并受总执行预算约束。
- [改进] A 股 Agent SSE 问股为单 Agent 补齐 `stage_start`/`stage_done` 生命周期事件，多 Agent 保持真实业务阶段事件。

- [新功能] 增加 A 股 DecisionSignal 按交易日的后验结果表、评估服务与查询 API。
...
- [修复] 修复股票自动补全外部受控值同步时会触发搜索并重新展开候选的问题，避免提醒页预填股票后自动弹出下拉。
- [改进] 个股详情页进入提醒规则时自动携带股票代码和名称，提醒表单可直接预填当前股票。
- [修复] 修复 Web 订单退款弹窗备注输入框 placeholder 颜色类名未生效的问题。
- [修复] 补齐 Web Tailwind surface 颜色令牌，恢复任务、提醒、报告等页面中 `bg-surface*` 背景与进度条轨道样式。
- [修复] 修复 Web 未登录侧边栏帮助入口跳转登录页的问题，并修正登录/注册页左侧说明文字在浅色主题下对比度过低的问题。
- [改进] 优化 Web 研报与公告公开页体验：研报支持 `?id=` 定位、移动端点选后滚动到详情、未登录购买可回跳登录，公开侧边栏收敛为研报/公告/帮助入口，并补齐公开页标题。
- [改进] Web 首页报告视图改为按需加载，减少未打开报告时的图表与 Markdown 渲染依赖；登录相关页面背景装饰收敛为轻量纹理。
- [改进] Web 侧边栏导航在 hover/focus 时预加载目标页面模块，减少懒加载路由切换等待。
- [改进] Web 研报页切换选中研报时不再重复请求列表，并避免快速切换时旧详情覆盖新详情。
- [改进] Web 帮助中心新增常用入口区，集中跳转任务中心、提醒规则、账务中心和研报广场。
- [改进] Web 账户页新增账务中心快捷入口，集中跳转订单、发票申请和会员积分管理。
- [改进] Web 账户页新增今日配额卡片，展示分析和问股当日用量、剩余次数及超限升级入口。
- [改进] Web 账户页新增套餐续费提醒卡片，展示到期状态、剩余天数和续费入口。
- [新功能] Web 账户页新增个人数据导出申请和账号注销冷静期管理入口。
- [新功能] Web 新增 `/usage` 用量看板，管理员可按今天、本月、全部查看 LLM 调用次数、Token 消耗和模型分布。
- [新功能] Web 新增 `/tasks` 任务中心页面，支持查看异步分析任务统计、状态筛选、进度和失败原因。
- [改进] Web 路由改为按页面懒加载，并拆分 React、图表、Markdown 等 vendor chunk，降低首屏入口包体积。
- [新功能] Web 新增 `/stocks/:code` 股票详情页，展示实时行情、历史走势和自选/问股/提醒入口。
- [新功能] Web 新增 `/alerts` 提醒规则页面，支持创建价格穿越、涨跌幅和放量提醒，并查看触发历史与通知投递记录。
- [修复] 修复账户页模型偏好下拉未读取后台保存的 LLM 渠道模型列表的问题，用户现在可在套餐允许范围内选择平台后台配置的模型。
- [修复] 修复账户页模型偏好在可选模型列表为空时被整体禁用的问题，保留“跟随平台默认”选项可操作。
- [新功能] Web 个人中心新增个人资料编辑能力，支持修改昵称和头像 URL，并在侧边栏展示用户头像与昵称。
- [chore] 新增 `scripts/seed_research_reports.py`，支持向当前配置数据库写入研报测试数据，并提供 dry-run、数量控制和同标题替换选项。
...
- [改进] Web 前端重构 Phase 6 继续推进：`tailwind.config.js` 新增 `shadow-card` → `var(--shadow-card)` 扩展；settings 组件群（`SettingsSectionCard`、`SettingsPanelErrorBoundary`、`SettingsLoading`、`SettingsCategoryNav`、`SettingsAlert`、`SettingsHelpButton`、`SettingsField`、`AuthSettingsCard`、`IntelligentImport`、`LLMChannelEditor`、`NotificationTestPanel`）全面迁离 `settings-border`/`settings-surface-*`/`settings-skeleton-*`/`settings-nav-item-active`/`settings-accent-*`/`settings-drag-active`/`settings-input-checkbox`/`shadow-soft-card*` CSS 类，替换为 `border-border/60|/80`、`bg-card/94|/97`、`shadow-card`、`bg-background/15`、`animate-pulse bg-foreground/10|/5`、`text-primary`、`accent-primary` 等语义化 Tailwind 类；`ChatPage`、`StickyActionBar`、`Collapsible`、`StatCard`、`Loading` 中 `shadow-soft-card*` 同步替换为 `shadow-card`；`StatCard` 和 `Loading` 图标 `text-cyan` 替换为 `text-primary`；从 `index.css` 删除 `shadow-soft-card`/`shadow-soft-card-strong` 类和全部 `settings-*` CSS 规则（约 145 行）；5 个 settings 组件测试文件 54 条通过，lint/build 通过。
- [改进] Web 前端重构 Phase 6 启动：删除 `index.css` 中全部 `btn-primary`/`btn-secondary` CSS 块（共 65 行）；`App.tsx` 网络错误重试按钮从 `btn-primary` 原生 button 替换为 `Button variant="primary"` 组件，加载 spinner `border-cyan` 替换为 `border-primary`；`BillingPage` 套餐卡、`OrdersPage` 状态色、`LegalPageLayout` 导航 tab、legal 三页邮件链接中所有 `text-cyan`/`border-cyan`/`bg-cyan` 替换为 `text-primary`/`border-primary`/`bg-primary`；`cn.test.ts` 移除 `btn-primary` 字符串引用；lint 0 errors，cn/LoginPage 定向测试通过，build 通过。
...
- [新功能] Phase 6 个人数据导出（PIPL 合规）：新增 `src/users/data_export.py` 收集用户注册信息、自选股、通知偏好、订阅历史、订单、协议同意记录并通过邮件发送 JSON 导出包；`POST /api/v1/account/data-export` 端点已上线；操作写 `app_audit_logs`。
...
- [文档] `.env.example` 补充 `PAYMENT_CALLBACK_ALLOWED_IPS*`、`PAYMENT_CALLBACK_SIG_FAIL_THRESHOLD`、`PAYMENT_CALLBACK_SIG_FAIL_WINDOW_SECONDS` 三组新变量说明；`docs/to-c-product-plan.md` Phase 5 安全收尾项标记为已落地。
- [测试] `tests/test_payment_gateway.py` 新增 `TestCheckCallbackIP`（6 条）与 `TestSigFailureAlert`（4 条），覆盖未配置放行、CIDR 范围、X-Forwarded-For 优先、per-provider key 优先级、精确单 IP、阈值触发告警、冷却防重复、不同 provider 独立计数等关键路径，10 条全部通过。
- [chore] 拆分 `src/storage.py`（原 ~2790 行）为 `src/storage/` 包：`base.py` 共享 `Base`，`models/{core,backtest,portfolio,conversation,alert,app}.py` 按业务域分散 ORM 表（共 38 张），`manager/{_base,stock_data,news,analysis_history,conversation,llm_usage,manager}.py` 通过 Mixin 组合出 `DatabaseManager`；`__init__.py` 全量 re-export 所有公共符号与 `get_db` / `persist_llm_usage`，保持 `from src.storage import ...` 完全兼容；同步修正 `tests/test_news_intel.py` 中 `patch("src.storage.time.sleep")` 为新模块路径 `src.storage.manager._base.time.sleep`。
- [新功能] Phase 6 §5.8.1 注册防刷护栏：新增 `src/users/registration_guard.py`，在 `register_user` 创建账号前依次执行一次性邮箱黑名单拦截（内置常见 disposable 域名 + `USER_DISPOSABLE_EMAIL_DOMAINS` 扩展 / `USER_DISPOSABLE_EMAIL_DOMAINS_REPLACE` 替换）和滚动窗口频率限制（IP 默认 10 次/24h、邮箱默认 3 次/24h，可通过 `USER_REGISTER_IP_DAILY_MAX` / `USER_REGISTER_EMAIL_DAILY_MAX` / `USER_REGISTER_RATE_WINDOW_HOURS` 调节，设为 0 关闭）；尝试 / 拦截事件均写入 `app_audit_logs`（`auth.register.attempt` / `auth.register.blocked`），邮箱按 SHA-256 哈希落库避免泄露，限流与黑名单统一返回 `rate_limited` / `invalid_email` 防止穷举区分。
- [测试] 新增 `tests/test_users_registration_guard.py`（14 条）覆盖一次性邮箱拦截、env 黑名单追加 / 替换、IP / 邮箱限频、限频关闭语义、`register_user` 端到端集成、常见消费级邮箱 (gmail/outlook/163/qq 等) 不被内置黑名单误伤等关键路径。
- [文档] `docs/to-c-mode.md` §2 补充注册防刷相关 7 个环境变量说明；`.env.example` 同步追加注册防刷护栏区段；`docs/to-c-product-plan.md` 进度速览将 Phase 6 注册防刷一项标记为已落地。
- [新功能] Phase 3 通知收尾：新增 `src/users/notification_delivery.py` 投递层，每日推送邮件由 `markdown2` 渲染为 HTML（含报告卡片、免责声明与一键退订链接），并按用户 `webhook_type` 分发到飞书 / 企业微信 / Discord / Telegram / 通用 JSON 五种渠道；调度器 `run_per_user_scheduled_analysis` 在邮件之后串接 Webhook 推送（仅 `plan.can_webhook=True` 且配置完整时触发），单个渠道失败仅日志不影响主链路。
- [新功能] 新增无状态退订机制：`src/users/unsubscribe.py` 通过 HMAC 签名签发 90 天有效 token，新增 `GET /api/v1/account/notification-prefs/unsubscribe` 公开端点（已加入 `AuthMiddleware` `EXEMPT_PATHS`），点击邮件链接即可关闭 `daily_push_enabled` 或 `email_enabled`，操作写入 `app_audit_logs`（`action='notification.unsubscribe'`）；签名密钥按 `UNSUBSCRIBE_SIGNING_KEY` → `ADMIN_API_SECRET` 优先级回退，邮件链接 base URL 通过 `USER_PUBLIC_BASE_URL` / `PUBLIC_BASE_URL` / `APP_BASE_URL` 三选一注入。
- [测试] 新增 `tests/test_unsubscribe.py`（8 条）与 `tests/test_notification_delivery.py`（14 条）覆盖 token 签发/校验/篡改/过期/密钥轮换、HTML 邮件含退订链接与免责声明、Webhook 按 `webhook_type` 分发到正确 schema、免费档 / 未配置渠道时静默 skip、`requests.post` 异常被吞掉返回 False 等关键路径。
...
- [文档] 新增 `docs/to-c-mode.md` 说明默认启用的 To C 多用户认证、环境变量、邮件后端、配额骨架与数据隔离边界；本次扩充第 7、8 节描述新前端入口与 quota_guard 接入点。
- [文档] 新增 `docs/to-c-product-plan.md` 锁定 To C 化产品总纲：用户分层与权益、配额数值、6 阶段路线、风险点与待确认事项；本次更新进度速览, Phase 2 升级为「主要落地」、Phase 4 升级为「部分落地」。
- [新功能] Phase 5 商业化数据层落地：`src/storage.py` 新增 `AppOrder` / `AppPaymentEvent` / `AppRefund` / `AppInvoice` 四张 ORM 表；新增 `src/services/billing/order_service.py` 服务层（订单创建/幂等/状态机流转、退款申请、发票申请、支付回调落库）；`/api/v1/billing/` 扩充 orders / pay / cancel / callbacks/wechat / callbacks/alipay / refunds / invoices 等端点，支持 `PAYMENT_ENABLED` 开关，关闭时返回 503 并附人工收款兜底提示。
...
- [修复] To C 数据多租户补全：`/api/v1/portfolio/*` 所有端点注入 `current_user`，已登录用户创建/读取/修改/删除均自动以 `str(current_user.id)` 作为 `owner_id` 强校验，无需（也不接受）客户端传入 owner；`portfolio_service` 写操作新增 `owner_id_check`、读操作与 snapshot/risk/fx-refresh 新增 `owner_id` 过滤；`portfolio_repo` 新增 `get_trade/cash_ledger/corporate_action_account_id` 辅助方法用于 delete 归属预校验；CSV 导入端点前置账户归属验证。
- [修复] 移除业务 API 的单用户兼容放行：`ENABLE_USER_REGISTRATION` 不再作为总开关，非豁免 `/api/v1/*` 业务接口只接受 `dsa_user_session`；analysis/history/agent/alerts/portfolio 强制使用当前用户上下文，quota_guard 对未登录调用直接报错而不再 bypass。
...
- [修复] To C 用户隔离补全：`alert_rules` 新增 `user_id` 并在告警 CRUD / 测试接口按当前登录用户隔离；Agent 对话 `conversation_messages` 新增 `user_id`，Web 登录态会话自动加 `u{user_id}:` 前缀并限制会话列表、读取与删除范围。
- [新功能] 新增 `src/users/model_router.py` 用户级模型路由骨架，统一解析套餐 `allowed_models`、`can_byok` 与用户 BYOK 凭据，为后续平台模型 / 用户自带 key 的统一调用链打基础。
- [新功能] 模型路由正式接入 LLM 调用链：`GeminiAnalyzer` 与 `LLMToolAdapter` 均通过 `_resolve_user_model_route` 在每次 LLM 调用前解析当前用户的 `ModelRoute`；BYOK 路由绕过 LiteLLM Router 直接调用并注入 `api_key` / `api_base`，平台路由按 `plan.allowed_models` 过滤可用模型列表；`build_agent_executor` 与 `StockAnalysisPipeline` 向下透传 `user_id`，API 层 `/agent/chat`、`/agent/chat/stream`、`/agent/research` 均已注入 `current_user.id`。
...
- [改进] 对账告警实现：`scripts/reconcile_payments.py` `_notify_diffs` 由纯日志升级为三通道：控制台日志 + 邮件（复用 `get_email_backend()`，收件人由 `ADMIN_ALERT_EMAIL` 环境变量控制）+ Webhook（`RECONCILE_WEBHOOK_URL`，兼容飞书/企业微信/通用 HTTP POST）；两个通道均可选，留空即跳过，失败静默告警不影响对账主流程。
- [文档] `docs/to-c-product-plan.md` §11.13 补充 `ADMIN_ALERT_EMAIL` / `RECONCILE_WEBHOOK_URL` 环境变量说明。
- [文档] 同步更新 `docs/to-c-product-plan.md`：Phase 5 商业化与 Phase 6 运营 / 合规 进度上调，标注协议三件套、运营后台、对账脚本骨架的落地状态与后续 SDK 接入位点。
- [新功能] Phase 5 真实支付 SDK 接入：新增 `src/services/billing/gateways/` 抽象层（`PaymentGateway` 基类 + `CallbackResult` / `ChannelSettlement` / `GatewayError` 数据结构 + `factory.get_gateway` / `has_gateway` 工厂），实现 `WechatGateway`（V3 RSA-SHA256 签名验签 + AES-256-GCM 解密）与 `AlipayGateway`（RSA2 SHA256 签名验签）；两路回调端点 `POST /callbacks/wechat` / `POST /callbacks/alipay` 接入验签后驱动 `OrderService.process_callback` 完成幂等去重 + `fulfill_order`；`approve_refund` 调用 `gateway.refund()` 拿通道退款单号，不传 `provider_refund_no` 且 gateway 未配置时回退人工模式；`issue_invoice` 在审批通过后复用 `src/users/email` 后端发送邮件回执。
- [改进] `scripts/reconcile_payments.py` `fetch_channel_settlements` 接入 gateway facade，实现了通道账单拉取扩展点，未实现时静默降级并记录日志；`.env.example` 新增 `WECHAT_PAY_*` / `ALIPAY_*` / `PAYMENT_ENABLED` / `PAYMENT_MOCK_ENABLED` 完整注释区段。
- [测试] 新增 `tests/test_payment_gateway.py`（19 条）覆盖 `WechatGateway.verify_callback`（签名通过/篡改/时间戳偏差/缺失 header/CLOSED 状态）、`AlipayGateway.verify_callback`（签名通过/app_id 不一致/金额篡改/TRADE_CLOSED）、`OrderService.process_callback`（付款驱动 fulfill + 幂等去重 + 金额不一致阻断 + 签名无效不驱动）、`approve_refund`（显式 refund_no 跳过 gateway / 无 refund_no 调用 gateway / NotImplementedError 回退人工）、`issue_invoice` 邮件回执捕获、gateway factory env override 三条用例；全 19 条 pass，`tests/test_billing_account_api.py` 全 17 条 pass（无回归）。
- [新功能] Phase 5 支付下单 / 退款真实 SDK 接入：`WechatGateway` 新增商户私钥签名基础设施（`_load_merchant_private_key` / `_build_auth_header` / `_http_post_v3` / `_http_get_v3`）及 `place_order`（WeChat V3 Native 下单，返回 `code_url`）与 `refund`（`/v3/refund/domestic/refunds`，返回 `refund_id`）；`AlipayGateway` 新增 `_load_app_private_key` / `_build_sign_params` / `_sign_params` / `_call_openapi` 及 `place_order`（`alipay.trade.precreate`，返回 `qr_code`）与 `refund`（`alipay.trade.refund`）；factory 透传新增环境变量 `WECHAT_PAY_PRIVATE_KEY_PEM/PATH` / `WECHAT_PAY_CERT_SERIAL_NO` / `ALIPAY_APP_PRIVATE_KEY_PEM/PATH`；`/api/v1/billing/orders/{order_no}/pay` 在 `PAYMENT_ENABLED=true` 时改为调用 `gateway.place_order()` 返回真实 `code_url` / `qr_code`（密钥缺失时 503，通道调用失败时 502）；`.env.example` 补充私钥字段注释区段。
- [测试] `tests/test_payment_gateway.py` 新增 11 条测试覆盖 `WechatGateway.place_order`（成功 / 缺少私钥 / 缺少 notify_url / HTTP 错误）、`WechatGateway.refund`（成功 / HTTP 错误）、`AlipayGateway.place_order`（成功 / 缺少私钥 / API 错误）、`AlipayGateway.refund`（成功 / API 错误）；全部 30 条 pass，原 19 条无回归。
