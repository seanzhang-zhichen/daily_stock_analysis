...
- [改进] Web 前端重构 Phase 6 启动：删除 `index.css` 中全部 `btn-primary`/`btn-secondary` CSS 块（共 65 行）；`App.tsx` 网络错误重试按钮从 `btn-primary` 原生 button 替换为 `Button variant="primary"` 组件，加载 spinner `border-cyan` 替换为 `border-primary`；`BillingPage` 套餐卡、`OrdersPage` 状态色、`ApiKeysPage` 状态 badge、`LegalPageLayout` 导航 tab、legal 三页邮件链接中所有 `text-cyan`/`border-cyan`/`bg-cyan` 替换为 `text-primary`/`border-primary`/`bg-primary`；`cn.test.ts` 移除 `btn-primary` 字符串引用；lint 0 errors，cn/LoginPage 定向测试通过，build 通过。
...
- [新功能] 新增 To C 多用户体系骨架：默认启用 `app_users` / `app_user_sessions` / `app_user_email_verifications` / `app_user_usage_counters` / `app_user_byok_credentials` 等表，新增 `/api/v1/account/*` 邮箱密码注册/登录/找回密码 endpoint，前端 `/register` `/forgot-password` 路由及 `UserAuthPage` / `ForgotPasswordPage` 页面。
...
- [新功能] 新增 `/api/v1/account/redeem` 兑换码 endpoint 与 `/api/v1/account/api-keys` (GET/POST/DELETE) BYOK 管理 endpoint；`/api/v1/account/status` 现在同步返回 `plan` 解析快照（`canByok` / `canWebhook` / `expiresAt`），免费档限额改由 plan 渲染而非裸 env。
- [新功能] 前端新增 `/account` `/billing` `/account/api-keys` 页面、侧边栏 `QuotaIndicator` 配额条、全局 `QuotaExceededDialog` 监听 axios interceptor 派发的 `dsa:quota-exceeded` 事件并提供「升级 Pro」/「配置 BYOK」/「关闭」三档引导。
...
- [测试] 新增 `tests/test_users_quota_guard.py` 覆盖 quota_guard 的未登录调用错误、扣减、超额、不限额、refund 与 payload 字段语义；新增 `tests/test_billing_account_api.py` 覆盖 billing 套餐目录 / 订阅查询、兑换码升级、BYOK CRUD 与 free 用户的 BYOK 写保护。
...
- [文档] 新增 `docs/to-c-product-wireframes.md` 输出登录/注册、找回密码、账户设置、会员中心、BYOK API Key、顶栏配额条、超额引导和首次引导等关键页面线框；本次将 §3–§7 标记为「已落地」。
