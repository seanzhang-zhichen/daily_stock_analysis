...
"""
    ...
    def _init_litellm(self) -> None:
        ...
        # --- Direct key path: build Router for multi-key, or use single key ---
        keys = get_api_keys_for_model(litellm_model, config)
        direct_model_list: List[Dict[str, Any]] = []
        if keys:
        ...
        if len(direct_model_list) > 1:
            ...
            try:
                ...
                logger.debug("Analyzer LLM: Router constructor signature not compatible; using direct call fallback")
                self._router = None
            else:
                logger.info(
                    f"Analyzer LLM: Router initialized with {len(direct_model_list)} direct keys "
                    f"for {litellm_model}"
                )
                return
    ...
    def _call_litellm(
        ...
        *,
        ...
        for model in models_to_try:
            ...
            direct_router_model_list = getattr(self, "_direct_router_model_list", None) or []
            if direct_router_model_list and model == config.litellm_model and not use_channel_router:
                recovery_model_list = direct_router_model_list
