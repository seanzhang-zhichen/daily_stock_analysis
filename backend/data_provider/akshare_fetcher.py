...
# 新浪财经实时行情接口的域名部分（不含协议头），用于构造请求 URL
SINA_REALTIME_ENDPOINT = "hq.sinajs.cn/list"
# 腾讯财经实时行情接口的域名部分（不含协议头），用于构造请求 URL
TENCENT_REALTIME_ENDPOINT = "qt.gtimg.cn/q"
# AkShare 历史数据接口调用超时时间（秒），用于子进程超时控制
_AKSHARE_HISTORY_CALL_TIMEOUT = 30.0
# AkShare 子进程 join 时的宽限时间（秒），用于优雅终止子进程
_AKSHARE_TIMEOUT_PROCESS_JOIN_GRACE = 1.0
# AkShare 子进程启动方式，使用 "spawn" 以避免 fork 带来的潜在问题（如 macOS 默认 fork 可能导致的问题）
_AKSHARE_TIMEOUT_PROCESS_START_METHOD = "spawn"


# User-Agent 池，用于随机轮换，模拟不同浏览器和操作系统环境，降低被反爬识别的风险
USER_AGENTS = [
...
class AkshareFetcher(BaseFetcher):
    ...
    def _fetch_index_data(self, target, start_date: str, end_date: str) -> pd.DataFrame:
        ...
        if "pct_chg" not in result.columns:
            result["pct_chg"] = pd.to_numeric(result["close"], errors="coerce").pct_change(fill_method=None).fillna(0.0)
